 ...
 * in every tenants handler (`tenants.handler.ts` + `tenant-members.handler.ts`).
 *
 * This is a verbatim copy of `packages/module-ai/src/api/_utils/sort.ts`
 * per the oven-bug-sprint F-05-01 pattern — the BO rule IP-4 in
 * `docs/modules/todo/oven-bug-sprint/sprint-05-handler-typesafety.md`
 * requires each module duplicate the helper rather than importing
 * across package boundaries. When the helper eventually moves to
 * `@oven/module-registry/api-utils`, both copies collapse to one
 * import.
 *
 * Package-private on purpose. The helper MUST NOT be exported from
 * `packages/module-tenants/src/index.ts`.
 */
