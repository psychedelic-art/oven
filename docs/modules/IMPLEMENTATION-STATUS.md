...
| Unified Playground | DONE | `UnifiedAIPlayground` in `@oven/agent-ui` is the single source of truth (Tailwind, MUI-free, router-free, embed-compatible). Dashboard `/ai-playground` is a ~40-line MUI wrapper that imports it. 3-panel layout: target/config selector, center chat surface, Inspector/Eval/Trace. Reuses `ChatWidget`-lineage `MessageList` / `MessageInput` / `CommandPalette`. Slash commands fetched from `/api/chat-commands` and wired to `MessageInput` via `usePlaygroundCommands` (local `/clear`, `/help`, `/status`, `/model`, `/temperature`, `/export`; workflow-blocked set for `/agent`, `/tools`, `/skill`, `/mcp`, `/pin`, `/feedback`, `/search`, `/reset`). Workflow responses injected as assistant messages via `chat.appendMessage` (tagged `metadata.source === 'workflow'`). `ChatHeader` + `filterMessagesForDisplay` ported from newsan. Double-post bug fixed. |
