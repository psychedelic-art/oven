...
> **Name**: `@oven/module-ai`
> **Dependencies**: `module-registry`, `module-subscriptions`
...
Module AI Phase 1 is fully implemented and tested:

| Metric | Count |
|--------|-------|
| Database tables | 9 (aiProviders, aiModelAliases, aiVectorStores, aiUsageLogs, aiBudgets, aiBudgetAlerts, aiTools, aiGuardrails, aiPlaygroundExecutions) |
| Engine components | 7 (provider-registry, model-resolver, middleware, cost-calculator, encryption, guardrail-engine, usage-tracker) |
| API routes | 32+ (CRUD for all entities + generate/stream/embed/image/object/multimodal/transcribe/speech operations) |
| Unit tests | 102 passing across 10 suites |
| Built-in tools | 16+ seeded (embed, generate, image, object, multimodal, transcribe, speech) |
| Events emitted | 15 |
| Config keys | 8 |
| Permissions | 23 |

### Known Gaps (Non-Blocking for Phase 4)
- Missing handler registrations: multimodal, transcribe, speech handler files exist but NOT in `apiHandlers` map
- No vector store operation API endpoints (query/upsert/delete documents) — adapter has methods but no handlers
- Streaming output guardrails not enforced (middleware `wrapStream` skips output guardrail evaluation)
- Rate limiting not enforced (config key and column exist but no enforcement logic)
- Service catalog not auto-seeded (UC-7 says 6 services, seed.ts doesn't create them)
- Tenant provider resolution incomplete (providerRegistry.resolve() doesn't filter by tenantId)

### Test Suites (102 tests / 10 suites)
- cost-calculator, embed, generate, guardrail-engine, middleware, model-resolver, provider-registry, tool-registry, usage-tracker, vector-store-adapter
...
```typescript
// ai_providers
tenantId: integer('tenant_id'),  // nullable — platform-wide providers have no tenant
}, (table) => [
  index('aip_tenant_id_idx').on(table.tenantId),
  index('aip_slug_idx').on(table.slug),
  index('aip_provider_type_idx').on(table.providerType),
  index('aip_enabled_idx').on(table.enabled),
]);

// ai_model_aliases
}, (table) => [
  index('ama_alias_idx').on(table.alias),
  index('ama_provider_id_idx').on(table.providerId),
  index('ama_model_type_idx').on(table.modelType),
]);

// ai_tool_configs
}, (table) => [
  index('atc_tool_name_idx').on(table.toolName),
  index('atc_provider_id_idx').on(table.providerId),
]);
...
tenantId: integer('tenant_id'),
  ...
  index('aul_user_id_idx').on(table.userId),
  index('aul_agent_id_idx').on(table.agentId),
...
// ai_usage_budgets
  ...
  index('aub_period_idx').on(table.period),
]);
```
...
```typescript
chat: {
  description: 'Unified AI services layer — provider-agnostic access to LLMs, embeddings, image/video generation, speech, vector databases, and RAG. All via Vercel AI SDK.',
  capabilities: [
    'generate text', 'stream text', 'generate structured objects', 'embed text',
    'generate images', 'text-to-speech', 'speech-to-text', 'vector store operations', 'RAG queries',
  ],
  actionSchemas: [
    {
      name: 'ai.generate',
      description: 'Generate text using a language model',
      parameters: { model: { type: 'string' }, prompt: { type: 'string', required: true }, system: { type: 'string' }, temperature: { type: 'number' } },
      requiredPermissions: ['ai-tools.invoke'],
      endpoint: { method: 'POST', path: 'ai/generate' },
    },
    {
      name: 'ai.embed',
      description: 'Embed a text value into a vector',
      parameters: { value: { type: 'string', required: true }, model: { type: 'string' } },
      requiredPermissions: ['ai-tools.invoke'],
    ...
    },
    {
      ...
      parameters: { prompt: { type: 'string', required: true }, model: { type: 'string' }, size: { type: 'string' } },
      requiredPermissions: ['ai-tools.invoke'],
      endpoint: { method: 'POST', path: 'ai/images/generate' },
    },
    {
      name: 'ai.rag.ask',
      description: 'End-to-end RAG: retrieve context from vector store and generate answer',
      parameters: { store: { type: 'string', required: true }, question: { type: 'string', required: true }, model: { type: 'string' } },
      requiredPermissions: ['ai-tools.invoke'],
      endpoint: { method: 'POST', path: 'ai/rag/ask' },
    },
  ],
},
```
...
```typescript
configSchema: [
  { key: 'DEFAULT_LANGUAGE_MODEL', type: 'string', description: 'Default language model alias or ID', defaultValue: 'gpt-4o', instanceScoped: true },
  { key: 'DEFAULT_EMBEDDING_MODEL', type: 'string', description: 'Default embedding model', defaultValue: 'text-embedding-3-small', instanceScoped: true },
  { key: 'GLOBAL_RATE_LIMIT_RPM', type: 'number', description: 'Global requests per minute limit', defaultValue: 100, instanceScoped: false },
  { key: 'USAGE_LOG_RETENTION_DAYS', type: 'number', description: 'Days to retain usage logs', defaultValue: 90, instanceScoped: false },
],
```
...
events: {
  schemas: {
    'ai.provider.created': {
      ...
      slug: { type: 'string' }, providerType: { type: 'string' },
    },
    'ai.tool.invoked': {
      toolName: { type: 'string', required: true }, tenantId: { type: 'number' },
      providerId: { type: 'number' }, modelId: { type: 'string' },
      userId: { type: 'number' }, agentId: { type: 'number' }, status: { type: 'string' },
    },
    'ai.vectorStore.created': {
      id: { type: 'number', required: true }, tenantId: { type: 'number' },
      slug: { type: 'string' }, adapter: { type: 'string' },
    },
    'ai.usage.budgetWarning': {
      budgetId: { type: 'number', required: true },
      scope: { type: 'string' }, currentUsage: { type: 'number' },
      limit: { type: 'number' }, threshold: { type: 'number' },
    },
    'ai.usage.budgetExceeded': {
      budgetId: { type: 'number', required: true },
      scope: { type: 'string' }, currentUsage: { type: 'number' }, limit: { type: 'number' },
    },
  },
},
```
...
```typescript
export async function seedAi(db: any) {
  const modulePermissions = [
    { resource: 'ai-providers', action: 'read', slug: 'ai-providers.read', description: 'View AI providers' },
    { resource: 'ai-providers', action: 'create', slug: 'ai-providers.create', description: 'Add providers' },
    { resource: 'ai-providers', action: 'update', slug: 'ai-providers.update', description: 'Edit providers' },
    { resource: 'ai-providers', action: 'delete', slug: 'ai-providers.delete', description: 'Remove providers' },
    { resource: 'ai-providers', action: 'test', slug: 'ai-providers.test', description: 'Test connectivity' },
    { resource: 'ai-tools', action: 'read', slug: 'ai-tools.read', description: 'Browse tool catalog' },
    { resource: 'ai-tools', action: 'invoke', slug: 'ai-tools.invoke', description: 'Invoke AI tools' },
    ...
    { resource: 'ai-usage', action: 'read', slug: 'ai-usage.read', description: 'View usage logs' },
    { resource: 'ai-budgets', action: 'read', slug: 'ai-budgets.read', description: 'View budgets' },
    { resource: 'ai-budgets', action: 'create', slug: 'ai-budgets.create', description: 'Create budgets' },
    { resource: 'ai-budgets', action: 'update', slug: 'ai-budgets.update', description: 'Edit budgets' },
    { resource: 'ai-aliases', action: 'read', slug: 'ai-aliases.read', description: 'View model aliases' },
    { resource: 'ai-aliases', action: 'create', slug: 'ai-aliases.create', description: 'Create aliases' },
  ];
...
// GET /api/ai/providers — List AI providers with optional tenant filtering
export async function GET(request: NextRequest) {
  const params = parseListParams(request);
  const tenantId = request.headers.get('x-tenant-id');
  const conditions = [];
  if (tenantId) {
    // Show platform-wide (null tenant) + tenant-specific providers
    conditions.push(or(isNull(aiProviders.tenantId), eq(aiProviders.tenantId, Number(tenantId))));
  }
  if (params.filter?.enabled !== undefined) conditions.push(eq(aiProviders.enabled, params.filter.enabled));
  const where = conditions.length ? and(...conditions) : undefined;
  const [rows, [{ count }]] = await Promise.all([
    db.select().from(aiProviders).where(where).orderBy(asc(aiProviders.name)).offset(params.offset).limit(params.limit),
    db.select({ count: sql`count(*)` }).from(aiProviders).where(where),
  ]);
  // Mask API keys before returning
  const masked = rows.map(r => ({ ...r, apiKey: '••••••••' }));
  return listResponse(masked, 'ai-providers', params, Number(count));
}
```
